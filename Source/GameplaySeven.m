...
#import "GameplaySeven.h"

@implementation GameplaySeven {
...
- (void)retry {
    // reload this level
    [[CCDirector sharedDirector] replaceScene: [CCBReader loadAsScene:@"GameplaySeven"]];
}
...
-(void)ccPhysicsCollisionPostSolve:(CCPhysicsCollisionPair *)pair lemon:(CCNode *)nodeA wildcard:(CCNode *)nodeB
{
    ...
    if (energy > 5000.f)
    {
        [self lemonRemoved:nodeA];
    }
}

- (void)lemonRemoved:(CCNode *)lemon {
    // load particle effect
    CCParticleSystem *explosion = (CCParticleSystem *)[CCBReader load:@"LemonExplosion"];
    // make the particle effect clean itself up, once it is completed
    explosion.autoRemoveOnFinish = TRUE;
    // place the particle effect on the seals position
    explosion.position = lemon.position;
    // add the particle effect to the same node the seal is on
    [lemon.parent addChild:explosion];
    
    // finally, remove the destroyed seal
    [lemon removeFromParent];
}
