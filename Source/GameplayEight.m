...
#import "GameplayEight.h"

@implementation GameplayEight {
...
- (void)retry {
    // reload this level
    [[CCDirector sharedDirector] replaceScene: [CCBReader loadAsScene:@"GameplayEight"]];
}
...
-(void)ccPhysicsCollisionPostSolve:(CCPhysicsCollisionPair *)pair watermelon:(CCNode *)nodeA wildcard:(CCNode *)nodeB
{
    ...
    if (energy > 5000.f)
    {
        [self watermelonRemoved:nodeA];
    }
}

- (void)watermelonRemoved:(CCNode *)watermelon {
    // load particle effect
    CCParticleSystem *explosion = (CCParticleSystem *)[CCBReader load:@"WatermelonExplosion"];
    // make the particle effect clean itself up, once it is completed
    explosion.autoRemoveOnFinish = TRUE;
    // place the particle effect on the seals position
    explosion.position = watermelon.position;
    // add the particle effect to the same node the seal is on
    [watermelon.parent addChild:explosion];
