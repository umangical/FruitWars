...
-(void)ccPhysicsCollisionPostSolve:(CCPhysicsCollisionPair *)pair strawberry:(CCNode *)nodeA wildcard:(CCNode *)nodeB
{
    ...
    if (energy > 5000.f)
    {
        [self strawberryRemoved:nodeA];
    }
}

- (void)strawberryRemoved:(CCNode *)strawberry {
    // load particle effect
    CCParticleSystem *explosion = (CCParticleSystem *)[CCBReader load:@"StrawberryExplosion"];
    // make the particle effect clean itself up, once it is completed
    explosion.autoRemoveOnFinish = TRUE;
    // place the particle effect on the seals position
    explosion.position = strawberry.position;
    // add the particle effect to the same node the seal is on
    [strawberry.parent addChild:explosion];
