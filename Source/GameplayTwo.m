...
#import "GameplayTwo.h"

@implementation GameplayTwo {
...
- (void)retry {
    // reload this level
    [[CCDirector sharedDirector] replaceScene: [CCBReader loadAsScene:@"GameplayTwo"]];
}
...
-(void)ccPhysicsCollisionPostSolve:(CCPhysicsCollisionPair *)pair kiwi:(CCNode *)nodeA wildcard:(CCNode *)nodeB
{
    ...
    if (energy > 5000.f)
    {
        [self kiwiRemoved:nodeA];
    }
}

- (void)kiwiRemoved:(CCNode *)kiwi {
    // load particle effect
    CCParticleSystem *explosion = (CCParticleSystem *)[CCBReader load:@"KiwiExplosion"];
    // make the particle effect clean itself up, once it is completed
    explosion.autoRemoveOnFinish = TRUE;
    // place the particle effect on the seals position
    explosion.position = kiwi.position;
    // add the particle effect to the same node the seal is on
    [kiwi.parent addChild:explosion];
    
    // finally, remove the destroyed seal
    [kiwi removeFromParent];
}
