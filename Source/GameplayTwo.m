...
- (void)launchKiwi {
    ...
    CCNode* kiwi = [CCBReader load:@"Kiwi"];
    // position the penguin at the bowl of the catapult
    kiwi.position = ccpAdd(_catapultArm.position, ccp(16, 50));
    
    // add the penguin to the physicsNode of this scene (because it has physics enabled)
    [_physicsNode addChild:kiwi];
    
    // manually create & apply a force to launch the penguin
    CGPoint launchDirection = ccp(1, 0);
    CGPoint force = ccpMult(launchDirection, 8000);
    [kiwi.physicsBody applyForce:force];
    
    // ensure followed object is in visible are when starting
    self.position = ccp(0, 0);
    CCActionFollow *follow = [CCActionFollow actionWithTarget:kiwi worldBoundary:self.boundingBox];
    [_contentNode runAction:follow];
}
